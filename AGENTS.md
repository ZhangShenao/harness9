...
- **Planning（原生规划能力）**: Plan 是 Agent 原生能力（System Prompt 准则驱动，LLM 自主判断复杂任务先规划）+ PlanStore（Session 级权威状态，状态机校验）+ 写时检查点（plan_write 成功轮立即落盘 SQLite `session_plans`，崩溃窗口缩至单轮）+ 压缩免疫（引擎在每轮压缩视图末尾原样注入活跃 Plan，Compactor 零改动）+ 主/子代理双向隔离（每个 Sub-Agent 拥有委派级独立 PlanStore）+ 已开始条目保留（plan_write 部分更新自动合并：省略的 in_progress/completed 条目保留原序，pending/cancelled 可裁剪，防作弊校验不受影响）+ autoExecuting 自动续跑（plan_write 激活，停滞 3 次放弃，取消即停）
- **Sub-Agent（子代理委派）**: 主代理通过 `task` 工具把边界清晰的子任务委派给运行在隔离 Session 上的专门子代理（独立上下文 + 受限工具集 + 可选模型覆盖）；六内置子代理库（general-purpose 兜底（对标 Claude Code / DeepAgents，继承父全部可用工具与模型）/ explorer 只读探索 / researcher Web 调研 / implementer 实现 / reviewer 审查 / planner 规划，编译进二进制，`.harness9/agents/*.md` 同名文件可覆盖），支持文件式定义、前台/后台双模式、`@agent` 直跑、TaskTracker 后台任务管理；异步调度与协调控制（三平面架构：TaskController 控制平面实现 engine.TaskGate 轮边界门控 + task_status/task_wait/task_control 协调三工具 + Runner 数据平面；轮边界暂停不耗 MaxTurns、steer 以 user 消息持久化、cancel 经 execCtx 立即中断；task_wait join 聚合（超时返回快照非 error）；后台完成自动唤醒主代理（预算 10、用户输入重置、`HARNESS9_AUTOWAKE=false` 关闭、压缩窗口跳过）；委派准则注入 + 委派 nudge 引导批量探索委派 explorer，配套 glob/grep 一等搜索工具）；安全保障：task 家族四工具对子代理永久剥离（禁止递归 + 防越权操纵主 tracker）+ 权限只能更严不能扩权 + 上下文完全隔离 + 结果恰好注入一次
...
│   │   ├── manager.go               # Manager：Create/CreateWithRetry/Destroy/DestroyAll/ReapOrphans/ListAll；并发安全
│   │   ├── daemon.go                # ensureDaemonReady：daemon 探测 + 自动拉起 + 有界轮询（CreateWithRetry 的前置保障）
│   │   ├── daemon_darwin.go         # tryStartDaemon macOS 实现（open -a Docker，幂等）
│   │   ├── daemon_other.go          # tryStartDaemon 非 darwin stub（不自动拉起，短等待快速失败）
│   │   └── *_test.go                # 各组件单元测试（container mock + manager 并发测试 + daemon 轮询）
...
│   │   └── dataset/                 # 黄金数据集（go test 可直接运行，29 个用例）
│   │       ├── tool_calling_test.go # 工具调用准确性（8 用例：bash/read_file/write+read/L4 模糊匹配/并行工具/纯对话/glob/grep）
│   │       ├── planning_test.go     # Planning 完成率（5 用例：生成计划/不写文件/先规划后执行/只读探索/简单任务不强制规划）
...
│   │       ├── subagent_test.go     # Sub-Agent 协调平面（4 用例：后台启动返回 id/steer 状态机/task_wait 聚合/并行委派与 explorer 选择）
...
| **tools** | 工具注册表 + 内置工具（bash / read_file（offset/limit 分页）/ write_file / edit_file / glob（文件名模式匹配，`**` 跨层级 + mtime 优先排序 + ≤200 条结构化输出）/ grep（跨文件正则搜索，二进制（首 8KB 含 NUL）与 >10MB 跳过 + 单行 500 runes 截断 + 命中 50–500 行上限，glob 参数过滤文件名）/ plan_write / memory_write（add/update/remove + Precis 重建）/ memory_search（FTS5 检索 + 命中强化）/ web_search（DuckDuckGo，无 API Key）/ web_fetch（go-readability + html-to-markdown，Markdown 输出））+ 路径沙箱（safe_path.go，glob/grep 的 path 搜索根同经校验）+ SSRF 防护（web_safety.go） | ✅ |
...
| **evals** | 自动化评估框架：`ScriptedProvider`（确定性 mock）、`Assertion`（Hard/Soft 断言，8 种实现）、`RunCase`/`Suite`（最小化隔离引擎 + `recordingHook`）、`SetupHermeticEnv`（Hermetic CI 隔离）、`BuildReport`/`WriteJSON`/`WriteMarkdown`（评估报告）；`dataset/` 黄金数据集 29 用例（tool_calling/planning/subagent/context/error_handling/memory/compaction）；`.github/workflows/eval.yml` Quality Gate | ✅ |
...
| Sub-Agent | `subagent_test.go` | 后台启动返回 id、steer 状态机、task_wait 聚合、并行委派、explorer 选择 |
...
当前黄金数据集：29 个用例，覆盖 tool_calling（8）/ planning（5）/ subagent（4）/ context（4）/ error_handling（3）/ memory（2）/ compaction（3）。新 feature 只能**增加**用例，不能删除或降低覆盖率。
...
**自动化保障：** 执行 `/sync-docs` 命令自动扫描并补全缺失或过期的英文文档（详见 `.zcode/commands/sync-docs.md`）。
