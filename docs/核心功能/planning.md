...
- **写模式**（提供 `steps` 数组）：防作弊校验通过后，与既有条目合并写入 PlanStore（支持部分更新，见下文"已开始条目保留"）
...
LLM 增量更新计划时，常只提交仍在执行或新增的条目。若按纯全量替换处理，已开始的条目会从权威状态中丢失——已完成条目丢的是历史（进度计数缩水、计划文件只剩尾部条目），in_progress 条目丢的是正在进行的任务（autoExecuting 续跑失据）。为此 plan_write 在写入前执行合并（`mergeWithPreservedCompleted`）：

1. **按快照原有顺序遍历**：条目在新列表中出现 → 采用新版本；未出现但状态为 in_progress/completed → 原样保留；未出现且为 pending/cancelled → 丢弃（尚未开始的条目允许裁剪，放弃条目的正规途径是显式标记 cancelled）
2. **追加新条目**：新列表中快照没有的条目追加在末尾，保持提交顺序
3. **顺序稳定**：既有条目始终按首次创建顺序排列，TUI 编号跨更新不跳变

防作弊校验仍只针对 LLM 本次提交的条目——保留的历史条目不会绕过校验。
