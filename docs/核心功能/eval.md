...
CI 阶段  ──→ Eval（黄金数据集评估）  24 个用例 + Quality Gate
...
```
          ...
          │   量化 Agent 能力边界        │    24 个用例，PR 触发，失败阻断合并
          └──────────────┬──────────────┘
                         │
          ┌──────────────▼──────────────┐
...
| `planning` | `plan_generated` | plan_write 写入计划 |
| `planning` | `exploration_before_plan` | 先探索后规划，探索阶段不越权调用 write_file/edit_file |
| `planning` | `plan_then_execute` | 先生成计划再执行（完整 Planning 链路） |
| `planning` | `exploration_only` | 明确要求只分析时仅用只读工具，不做写操作 |
| `planning` | `simple_task_no_plan` | 简单任务不强制规划（规划是按需的原生能力） |
...
| `compaction` | `plan_survives` | 历史被压缩后活跃 Plan 仍原样出现在发送视图末尾（压缩免疫） |

### 3.2 运行 Eval

```bash
# 运行全量黄金数据集（24 个用例，无需 API Key）
...
- 当前 24 个用例是 baseline，只能增加，不能删除或降低覆盖率
