...
| **Stagnation nudge `WithStallNudge(10, …)`** | When 10 consecutive turns show no edits/test runs (only spinning on static re-reads/greps), the engine injects one nudge to break the idle loop (trajectory analysis R6: xarray-3364 and pylint-7080 burned through all 80 turns in exactly this pattern). Applies only to a temporary copy, not persisted |
...
| **Stagnation nudge `WithStallNudge`**: injects one nudge to break the idle loop when N consecutive turns show no edit/write progress tool calls (defensive copy, not persisted) | `engine/options.go` + `engine/nudge.go` | R6 |
